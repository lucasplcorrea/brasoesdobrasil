...
async function generateReviewHtml() {
  ...
  await writeTextAtomic(
    ...
    `<!doctype html><html lang="pt-BR"><meta charset="utf-8"><title>Revisão</title><style>body{font:16px system-ui;max-width:1000px;margin:auto}section{border-bottom:1px solid #ccc}article{display:inline-block;vertical-align:top;width:45%;padding:2%}img{object-fit:contain;background:#fff;border:1px solid #ddd}dt{font-weight:bold}</style><h1>Revisão humana</h1><p>Nenhum item nesta página está aprovado automaticamente.</p>${cards}</html>`,
  );
}
